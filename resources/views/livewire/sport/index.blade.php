<div>
    <div class="card-controls sm:flex">
        <div class="w-full sm:w-1/2">
            Per page:
            <select wire:model="perPage" class="form-select w-full sm:w-1/6">
                @foreach($paginationOptions as $value)
                    <option value="{{ $value }}">{{ $value }}</option>
                @endforeach
            </select>

            @can('sport_delete')
                <button class="btn btn-rose ml-3 disabled:opacity-50 disabled:cursor-not-allowed" type="button" wire:click="confirm('deleteSelected')" wire:loading.attr="disabled" {{ $this->selectedCount ? '' : 'disabled' }}>
                    {{ __('Delete Selected') }}
                </button>
            @endcan

            {{-- @if(file_exists(app_path('Http/Livewire/ExcelExport.php')))
                <livewire:excel-export model="Sport" format="csv" />
                <livewire:excel-export model="Sport" format="xlsx" />
                <livewire:excel-export model="Sport" format="pdf" />
            @endif --}}




        </div>
        <div class="w-full sm:w-1/2 sm:text-right">
            Search:
            <input type="text" wire:model.debounce.300ms="search" class="w-full sm:w-1/3 inline-block" />
        </div>
    </div>
    <div wire:loading.delay>
        Loading...
    </div>

    <div class="overflow-hidden">
        <div class="overflow-x-auto">
            <table class="table table-index w-full">
                <thead>
                    <tr>
                        <th class="w-9">
                        </th>
                        <th class="w-28">
                            {{ trans('cruds.sport.fields.id') }}
                            @include('components.table.sort', ['field' => 'id'])
                        </th>
                        <th>
                            {{ trans('cruds.sport.fields.code') }}
                            @include('components.table.sort', ['field' => 'code'])
                        </th>
                        <th>
                            {{ trans('cruds.sport.fields.name') }}
                            @include('components.table.sort', ['field' => 'name'])
                        </th>
                        <th>
                            {{ trans('cruds.sport.fields.description') }}
                            @include('components.table.sort', ['field' => 'description'])
                        </th>
                        <th>
                            {{ trans('cruds.sport.fields.creator') }}
                            @include('components.table.sort', ['field' => 'creator.name'])
                        </th>
                        <th>
                            {{ trans('cruds.sport.fields.col_player_per_team') }}
                            @include('components.table.sort', ['field' => 'min_player_per_team']) 
                        </th>
                        <th>
                            {{ trans('cruds.sport.fields.is_require_choose_role') }}
                            @include('components.table.sort', ['field' => 'is_require_choose_role'])
                        </th>
                        <th>
                            {{ trans('cruds.sport.fields.updated_at') }}
                            @include('components.table.sort', ['field' => 'updated_at'])
                        </th>
                        <th>
                        </th>
                    </tr>
                </thead>
                <tbody>
                    @forelse($sports as $sport)
                        <tr>
                            <td>
                                <input type="checkbox" value="{{ $sport->id }}" wire:model="selected">
                            </td>
                            <td>
                                {{ $sport->id }}
                            </td>
                            <td>
                                {{ $sport->code }}
                            </td>
                            <td>
                                {{ $sport->name }}
                            </td>
                            <td>
                                {{ $sport->description }}
                            </td>
                            <td>
                                @if($sport->creator)
                                    <span class="badge badge-relationship">{{ $sport->creator->email ?? '' }}</span>
                                @endif
                            </td>
                            <td>
                                {{ $sport->min_player_per_team }} to {{ $sport->max_player_per_team }} 
                            </td>
                            <td>
                                {{ $sport->is_require_choose_role_label }}
                            </td>
                            <td>
                                {{  Carbon\Carbon::parse($sport->updated_at)?->format('M-d-Y h:i:s A') ?? '__' }}
                            </td>
                            <td>
                                <div class="flex justify-end">
                                    @can('sport_show')
                                        <a class="btn btn-sm btn-info mr-2" href="{{ route('admin.sports.show', $sport) }}">
                                            {{ trans('global.view') }}
                                        </a>
                                    @endcan
                                    @can('sport_edit')
                                        <a class="btn btn-sm btn-success mr-2" href="{{ route('admin.sports.edit', $sport) }}">
                                            {{ trans('global.edit') }}
                                        </a>
                                    @endcan
                                    @can('sport_delete')
                                        <button class="btn btn-sm btn-rose mr-2" type="button" wire:click="confirm('delete', {{ $sport->id }})" wire:loading.attr="disabled">
                                            {{ trans('global.delete') }}
                                        </button>
                                    @endcan
                                </div>
                            </td>
                        </tr>
                        @empty
                        <tr>
                            <td colspan="10">No entries found.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
        </div>
    </div>

    <div class="card-body">
        <div class="pt-3">
            @if($this->selectedCount)
                <p class="text-sm leading-5">
                    <span class="font-medium">
                        {{ $this->selectedCount }}
                    </span>
                    {{ __('Entries selected') }}
                </p>
            @endif
            {{ $sports->links() }}
        </div>
    </div>
</div>

@push('scripts')
    <script>
        Livewire.on('confirm', e => {
    if (!confirm("{{ trans('global.areYouSure') }}")) {
        return
    }
@this[e.callback](...e.argv)
})
    </script>
@endpush