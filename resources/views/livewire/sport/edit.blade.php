<form wire:submit.prevent="submit" class="pt-3">
    ...
    <div class="form-group {{ $errors->has('mediaCollections.sport') ? 'invalid' : '' }}">
        <label class="form-label" for="sport">{{ trans('cruds.sport.fields.icon') }}</label>
        <x-dropzone id="icon" name="icon" action="{{ route('admin.sports.storeMedia') }}" collection-name="sport" max-file-size="2" max-width="4096" max-height="4096" max-files="1" />
        <div class="validation-message">
            {{ $errors->first('mediaCollections.sport') }}
        </div>
        <div class="help-block">
