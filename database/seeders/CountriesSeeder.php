...
class CountriesSeeder extends Seeder
{
     ...
     * @return void
     */
    public function run(IsoCodesFactory $isoCodesFactory)
    {
        $now = now();

        $phonesCodes = $this->getPhonesCodes();
        $countries = array_map(function (Country $country) use ($now, $phonesCodes) {
            return [
                'name' => $country->getName(),
                'code' => $country->getAlpha2(),
                'phone_code' => $phonesCodes[$country->getAlpha2()]['code'] ?? null,
                'created_at' => $now,
                'updated_at' => $now,
            ];
        }, $isoCodesFactory->getCountries()->toArray());

        $countries = array_filter($countries, function (array $country) {
            return $country['phone_code'] !== null;
        });

        DB::table('countries')->insert($countries);
    }
    protected function getPhonesCodes(): array
    {
        return [
